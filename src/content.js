...
chrome.storage.sync.get(["minTime_m", "minTime_s", "maxTime_m", "maxTime_s"], (result) => {
    window.config = {
        minTime:
            (result.minTime_m * TimeConstants.MIN_TO_S + result.minTime_s) * TimeConstants.S_TO_MS ||
            1 * TimeConstants.MIN_TO_S * TimeConstants.S_TO_MS, // default to 60 seconds if not set
        maxTime:
            (result.maxTime_m * TimeConstants.MIN_TO_S + result.maxTime_s) * TimeConstants.S_TO_MS ||
            5 * TimeConstants.MIN_TO_S * TimeConstants.S_TO_MS, // default to 5 minutes if not set
    };
    randomPopupTrigger();
});
