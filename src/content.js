    ...
    window.config = {
      minTime:
        (result.minTime_m * Constants.MIN_TO_S + result.minTime_s) *
          Constants.S_TO_MS || 1 * Constants.MIN_TO_S * Constants.S_TO_MS, // default to 60 seconds if not set
      maxTime:
        (result.maxTime_m * Constants.MIN_TO_S + result.maxTime_s) *
          Constants.S_TO_MS || 5 * Constants.MIN_TO_S * Constants.S_TO_MS, // default to 5 minutes if not set
    };
    randomPopupTrigger();
  },
);
